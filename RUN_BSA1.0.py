...
# this makes pylint stop compalaining about brackets
# around print and return
import sys
import argparse
import math
...
PARSER.add_argument("-hex", "--hex", required=False,action="store_true")
...
PARSER.add_argument("-het", "--heterozygous", required=False, action="store_true",
                    help="Both parents hets")
...
if ARGIES.min_allele:
    ...
    ARGDICT["min_allele"] = ARGDICT["window"]/1000
...
if ARGIES.hex:
    ARGDICT["color"] = ["#"+i if i != "custom" else i for i in ARGDICT["color"]]
...
if len(ARGDICT["color"]) == 1:
    ...
    if not ARGIES.hex:
        ...
        INVALID_COLORS = [cc for cc in ARGDICT["color"][1:] if cc not in ALL_COLORS_NAME]
        if INVALID_COLORS:
...
if ARGIES.heterozygous:
    ARGDICT["heterozygous"] = ARGIES.heterozygous
...
def haplodiploid(indv):
    ...
    if (len(exp_genosplito | cont_genosplito) == 2
        ...
        if (bothdiff or
                (not bothdiff
                 and sample_score < ARGDICT["mac"])):
            if reverse:
                sample_score = 1 - sample_score
            return(sample_score)

def both_het(indv):
    """Process offspring information when both parents hets"""
    exp_genosplito = set(indv[1].split(":")[0].split("/"))
    cont_genosplito = set(indv[2].split(":")[0].split("/"))
    if (len(exp_genosplito | cont_genosplito) == 2
            and exp_genosplito != cont_genosplito
        ...
        if len(exp_genosplito) == 1 and len(cont_genosplito) == 1:
            ...
            exp_allele = list(exp_genosplito)[0]
            reverse = False
        else: # heterozygosity in the control parent
            bothdiff = False
            if len(exp_genosplito) == 1:
                exp_allele = list(exp_genosplito)[0]
                reverse = False
        ...
        if (bothdiff or
                (not bothdiff
                 and sample_score < ARGDICT["mac"])):
            sample_score = sample_score+(1-sample_score)*0.5
            if reverse:
                sample_score = 1 - sample_score
            return(sample_score)
...
def process_samples(line, vcfline, cov):
    ...
    for sample in zip(ARGDICT["selected_offspring"]+ARGDICT["control_offspring"],
        ...
        if (len([call for call in indv if "." not in call]) == 3 and
                ...
                ########### NOW ONTO THE ACTUAL CALLS
            if "heterozygous" in ARGDICT:
                sample_score = both_het(indv)
            elif "haplodiploid" in ARGDICT:
                sample_score = haplodiploid(indv)
            else:
...
def finish_plot(axes, indict, fig, commas):
    ...
    if "nvr" in indict[all_spls[0]]:
        ...
        axes.set_ylabel("Change in allele frequency", size=17.24)
    else:
...
if ("selected_offspring" in ARGDICT
    ...
    if "heterozygous" in ARGDICT and "haplodiploid" in ARGDICT:
        error("CANNOT BE BOTH HETEROZYGOUS AND HAPLODIPLOID")

    # finds average genome-wide read coverage for each strain/population in VCF file
    coverage()
    print("elapsed %s min"%((time.clock()-START)/60.0))
    START = time.clock()
